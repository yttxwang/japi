	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		function ezSQL_mysql($dbuser=DB_USER, $dbpassword=DB_PASSWORD, $dbname=DB_DATABASE, $dbhost=DB_HOST)
		{
			$this->dbuser = $dbuser;
			$this->dbpassword = $dbpassword;
			$this->dbname = $dbname;
			$this->dbhost = $dbhost;
			$this->cache_timeout = CACHE_LIFETIME; // Note: this is hours
			$this->cache_dir = CONF_PATH_CACHE; 	// Specify a cache dir. Path is taken from calling script
		}
		...
		*/
		...
		function query($query)
		{
			...
			if ( $str = @mysql_error($this->dbh) )
			{
				...
				error_log('====>'.$query.'||==>'.print_r($aa,1), 3, "/data/application/logs/japi/default/mysql-errors.log");
