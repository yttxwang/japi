	...
	class ezSQLcore
	{
		...
		*/
		...
		function store_cache($query,$is_insert)
		{
			...
			{
				$cache_file = $this->buid_dir($query,true);//子目录缓存
		...
		*/
		...
		function get_cache($query)
		{
			...
			// The would be cache file for this query
			//$cache_file = $this->cache_dir.'/'.md5($query);
			$cache_file = $this->buid_dir($query,false);
		...
		function buid_dir ($query, $isbuild, $mode=0777)
		{
			$md5msg  = md5($query);
			$fheader = substr($md5msg,0,1);
			
			$b_path  = $this->cache_dir.'/'.$fheader;
