...
class RSSCreator10 extends FeedCreator {
	...
	function createFeed() {     
		...
		$now = new FeedDate();
		$feed.= "       <dc:date>".htmlspecialchars($this->date)."</dc:date>\n";
		$feed.= "        <items>\n";
		$feed.= "            <rdf:Seq>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			if ($this->items[$i]->date!=null) {
				...
				$feed.= "        <dc:date>".$this->items[$i]->date."</dc:date>\n";
			}
...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		if ($this->pubDate!="") {
			...
			$feed.= "        <pubDate>".$this->pubDate."</pubDate>\n";
		}
		if ($this->category!="") {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			...
			if ($this->items[$i]->date!="") {
				$itemDate = new FeedDate($this->items[$i]->date);
				$feed.= "            <pubDate>".$this->items[$i]->date."</pubDate>\n";
			}
